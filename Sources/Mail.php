...
class Mail
{
	 ...
	 */
	public static function addToQueue($flush = false, $to_array = array(), $subject = '', $message = '', $headers = '', $send_html = false, $priority = 3, $is_private = false)
	{
	...
	public static function reduceQueue($number = false, $override_limit = false, $force_send = false)
	{
		...
		foreach ($emails as $email)
		{
			...
			{
				$result = self::sendSmtp(array($email['to']), $email['subject'], $email['body'], $email['headers']);
			}
	 ...
	 */
	public static function sendNotifications($topics, $type, $exclude = array(), $members_only = array())
	{
