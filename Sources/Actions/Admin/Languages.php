...
class Languages extends AbstractAction
{
	...
	public function editEntries()
	{
		...
		Utils::$context['primary_settings']['name'] = Utils::ucwords(strtr($lang_id, array('_' => ' ', '-utf8' => '')));
				...
				foreach ($_POST['edit'] as $k => $v)
				{
					...
					elseif (is_array($v))
					{
						foreach ($v as $subk => $subv)
						{
							if (
								$subv == 'edit'
								&& isset($_POST['entry'][$k][$subk])
								&& isset($_POST['comp'][$k][$subk])
	...
	{
		$config_vars = array(
			'language' => array('language', Lang::$txt['default_language'], 'file', 'select', array(), null, 'disabled' => !is_writable(SMF_SETTINGS_FILE)),
