...
class Subscriptions extends AbstractAction
{
	...
	public static array $subactions = array(
		'view' => array('view', 'admin_forum'),
		'viewsub' => array('viewUsers', 'admin_forum'),
		'modify' => array('modify', 'admin_forum'),
		'modifyuser' => array('modifyUser', 'admin_forum'),
	...
	public static function getSubs(): array
	{
		...
		while ($row = Db::$db->fetch_assoc($request))
		{
			...
			self::$all[$row['id_subscribe']] = array(
				'id' => $row['id_subscribe'],
				'name' => $row['name'],
				'desc' => $row['description'],
				'cost' => $cost,
	 ...
	 */
	public static function list_getSubscribedUserCount($id_sub, $search_string, $search_vars = array()): int
	{
	 ...
	 */
	public static function list_getSubscribedUsers($start, $items_per_page, $sort, $id_sub, $search_string, $search_vars = array()): array
	{
		$subscribers = array();
	...
	protected function __construct()
	{
		...
		{
			self::$subactions = array_intersect_key(self::$subactions, array('settings' => true));
