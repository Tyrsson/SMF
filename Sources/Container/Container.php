...
class Container implements ContainerInterface
{
	...
	public function setAlias($alias, $target)
	{
		if (isset($this->services[$alias]) && ! $this->allowOverride) {
			throw Exception\ContainerModificationsNotAllowedException::fromExistingService($alias);
		}
	...
	private function resolveInitializers(array $initializers): void
	{
		foreach ($initializers as $initializer) {
			...
			throw Exception\InvalidArgumentException::fromInvalidInitializer($initializer);
		}
	}
	...
	private function createLazyServiceDelegatorFactory(): LazyServiceFactory
	{
		...
		if (! isset($this->lazyServices['class_map'])) {
			throw new Exception\ServiceNotCreatedException('Missing "class_map" config key in "lazy_services"');
		}
