...
class Bans implements ActionInterface
{
	...
	protected function banLoadAdditionalIPs($member_id): array
	{
		...
		$search_list += array(
			...
			'ips_in_errors' => array($this, 'banLoadAdditionalIPsError'),
		);
	 ...
	 */
	protected function removeBanLogs($ids = array()): bool
	{
		if (empty($ids))
		{
	 ...
	 */
	protected function addTriggers($group_id = 0, $triggers = array(), $logs = array()): bool
	{
		if (empty($group_id))
	 ...
	 */
	protected function updateTriggers($ban_item = 0, $group_id = 0, $trigger = array(), $logs = array()): void
	{
		if (empty($ban_item))
	 ...
	 */
	protected function updateBanGroup($ban_info = array()): int
	{
	 ...
	 */
	protected function insertBanGroup($ban_info = array()): int|false
	{
		if (empty($ban_info['name']))
			Utils::$context['ban_errors'][] = 'ban_name_empty';
	 ...
	 */
	protected static function removeBanTriggers($items_ids = array(), $group_id = null): bool
	{
		if (isset($group_id))
