...
class Theme
{
	 ...
	 */
	public array $agreement_actions = array(
		'agreement' => true,
	 ...
	 */
	public array $simpleActions = array(
		'findmember',
		'helpadmin',
		'printpage',
	);
	 ...
	 */
	public array $simpleAreas = array(
	 ...
	 */
	public array $simpleSubActions = array(
		'pm' => array('popup',),
	 ...
	 */
	public array $extraParams = array(
		'preview',
		'splitjs',
	);
	 ...
	 */
	public static array $reservedVars = array(
		'actual_theme_url',
		'actual_images_url',
		'base_theme_dir',
		'base_theme_url',
	...
	public static function loadEssential()
	{
		...
		{
			self::$current->settings['template_dirs'] = array(self::$current->settings['theme_dir']);
	 ...
	 */
	public static function loadTemplate($template_name, $style_sheets = array(), $fatal = true)
	{
		...
		if (!empty($style_sheets))
		{
			...
			foreach ($style_sheets as $sheet)
				self::loadCSSFile($sheet . '.css', array(), $sheet);
		}
	 ...
	 */
	public static function loadCSSFile($fileName, $params = array(), $id = '')
	{
		...
		if (!empty(Utils::$context['right_to_left']) && !empty($params['rtl']))
			self::loadCSSFile($params['rtl'], array_diff_key($params, array('rtl' => 0)));
	 ...
	 */
	public static function loadJavaScriptFile($fileName, $params = array(), $id = '')
	{
		$params['seed'] = (!array_key_exists('seed', $params) || (array_key_exists('seed', $params) && $params['seed'] === true)) ?
			(array_key_exists('browser_cache', Utils::$context) ? Utils::$context['browser_cache'] : '') :
			(is_string($params['seed']) ? '?' . ltrim($params['seed'], '?') : '');
	...
	public static function setupContext($forceload = false)
	{
		...
		{
			Utils::$context['meta_tags'][] = array('property' => 'og:image', 'content' => self::$current->settings['og_image']);
		}
	...
	public static function setupMenuContext()
	{
		...
		if (!User::$me->is_guest)
		{
			...
			if (!empty(Config::$modSettings['enable_ajax_alerts']))
			{
				...
				self::loadJavaScriptFile('alerts.js', array('minimize' => true), 'smf_alerts');
			}
		}
	...
	public static function setJavaScript()
	{
		...
		{
			self::$current->options['admin_preferences'] = !empty(self::$current->options['admin_preferences']) ? Utils::jsonDecode(self::$current->options['admin_preferences'], true) : array();
		...
		Db::$db->insert('replace',
			...
			array(self::$current->settings['theme_id'], User::$me->id, $_GET['var'], is_array($_GET['val']) ? implode(',', $_GET['val']) : $_GET['val']),
	...
	public static function pickTheme()
	{
		...
		if (isset($_POST['save']))
		{
			...
			elseif (self::canPickTheme($_REQUEST['u'], $id_theme))
			{
				...
				User::updateMemberData($_REQUEST['u'], array('id_theme' => $id_theme));
				...
				if (!empty($variant))
				{
					...
					Db::$db->insert('replace',
						'{db_prefix}themes',
						array('id_theme' => 'int', 'id_member' => 'int', 'variable' => 'string-255', 'value' => 'string-65534'),
						array($id_theme, $_REQUEST['u'], 'theme_variant', $variant),
						array('id_theme', 'id_member', 'variable')
					);
		...
		$current_theme_variants = !empty(self::$current->settings['theme_variants']) ? self::$current->settings['theme_variants'] : array();
			...
			if (file_exists($theme_data['theme_dir'] . '/index.template.php') && (empty($theme_data['disable_user_variant']) || User::$me->allowedTo('admin_forum')))
			{
				...
				if (preg_match('~((?:SMF\\\\)?Theme::\$current(?:->|_)|\$)settings\[\'theme_variants\'\]\s*=(.+?);~', $file_contents, $matches))
				{
					...
					if (!empty(self::$current->settings['theme_variants']))
					{
						...
						Utils::$context['available_themes'][$id_theme]['selected_variant'] = isset($_GET['vrt']) ? $_GET['vrt'] : (!empty($variant_preferences[$id_theme]) ? $variant_preferences[$id_theme] : (!empty(self::$current->settings['default_variant']) ? self::$current->settings['default_variant'] : self::$current->settings['theme_variants'][0]));
		...
		self::loadJavaScriptFile('profile.js', array('defer' => false, 'minimize' => true), 'smf_profile');
	 ...
	 * @param boolean $force_use Whether to override template_create_button and
	 *    use this instead.
	...
	protected function __construct($id = 0, $member = -1)
	{
		...
		elseif (($temp = CacheApi::get('theme_settings-' . $this->id, 90)) != null && time() - 60 > Config::$modSettings['settings_updated'])
		{
			...
			{
				$themeData[$member] = array();
			}
		}
		else
		{
			$themeData = array(-1 => array(), 0 => array());
		...
		if (empty($flag))
		{
			...
			$result = Db::$db->query('', '
				...
				array(
					'id_theme' => $this->id,
					'id_member' => $member,
					'themes' => array_unique(array(1, $this->id)),
					'members' => array_unique(array(-1, 0, $member)),
				)
			);
			...
			elseif (!isset($temp))
			{
				CacheApi::put('theme_settings-' . $this->id, array(-1 => $themeData[-1], 0 => $themeData[0]), 90);
			}
		}
		...
		$this->settings['template_dirs'] = array();
	...
	protected function initialize()
	{
		...
		if (!empty(Config::$modSettings['integrate_theme_include']))
		{
			...
			{
				$include = strtr(trim($include), array('$boarddir' => Config::$boarddir, '$sourcedir' => Config::$sourcedir, '$themedir' => $this->settings['theme_dir']));
	...
	protected function fixUrl()
	{
		...
		if (isset($detected_url) && $detected_url != Config::$boardurl)
		{
			...
			if (!empty($do_fix) || preg_match('~^http[s]?://(?:[\d\.:]+|\[[\d:]+\](?::\d+)?)(?:$|/)~', $detected_url) == 1)
			{
				...
				// Fix the theme urls...
				$this->settings['theme_url'] = strtr($this->settings['theme_url'], array($oldurl => Config::$boardurl));
				$this->settings['default_theme_url'] = strtr($this->settings['default_theme_url'], array($oldurl => Config::$boardurl));
				$this->settings['actual_theme_url'] = strtr($this->settings['actual_theme_url'], array($oldurl => Config::$boardurl));
				$this->settings['images_url'] = strtr($this->settings['images_url'], array($oldurl => Config::$boardurl));
				$this->settings['default_images_url'] = strtr($this->settings['default_images_url'], array($oldurl => Config::$boardurl));
				$this->settings['actual_images_url'] = strtr($this->settings['actual_images_url'], array($oldurl => Config::$boardurl));
	...
	protected function loadTemplatesAndLangFiles()
	{
		...
		Utils::$context['simple_action'] = (
			...
			|| (
				isset($this->simpleAreas[Utils::$context['current_action']])
				&& isset($_REQUEST['area'])
	...
	protected function loadCss()
	{
		...
		self::loadCSSFile('index.css', array('minimize' => true, 'order_pos' => 1), 'smf_index');

		// Here is my luvly Responsive CSS
		self::loadCSSFile('responsive.css', array('force_current' => false, 'validate' => true, 'minimize' => true, 'order_pos' => 9000), 'smf_responsive');

		if (Utils::$context['right_to_left'])
			self::loadCSSFile('rtl.css', array('order_pos' => 4000), 'smf_rtl');
	}
	...
	protected function loadVariant()
	{
		...
		if (!empty($this->settings['theme_variants']))
		{
			...
			{
				self::loadCSSFile('index' . Utils::$context['theme_variant'] . '.css', array('order_pos' => 300), 'smf_index' . Utils::$context['theme_variant']);

				if (Utils::$context['right_to_left'])
				{
					self::loadCSSFile('rtl' . Utils::$context['theme_variant'] . '.css', array('order_pos' => 4200), 'smf_rtl' . Utils::$context['theme_variant']);
				}
			}
		}
	}
	...
	protected function loadJavaScript()
	{
		...
		{
			self::loadJavaScriptFile($jQueryUrls[Config::$modSettings['jquery_source']], array('external' => true, 'seed' => false), 'smf_jquery');
		}
		elseif (isset(Config::$modSettings['jquery_source']) && Config::$modSettings['jquery_source'] == 'local')
		{
			self::loadJavaScriptFile('jquery-' . JQUERY_VERSION . '.min.js', array('seed' => false), 'smf_jquery');
		}
		elseif (isset(Config::$modSettings['jquery_source'], Config::$modSettings['jquery_custom']) && Config::$modSettings['jquery_source'] == 'custom')
		{
			self::loadJavaScriptFile(Config::$modSettings['jquery_custom'], array('external' => true, 'seed' => false), 'smf_jquery');
		}
		// Fall back to the forum default
		else
		{
			self::loadJavaScriptFile('https://ajax.googleapis.com/ajax/libs/jquery/' . JQUERY_VERSION . '/jquery.min.js', array('external' => true, 'seed' => false), 'smf_jquery');
		}

		// Queue our JQuery plugins!
		self::loadJavaScriptFile('smf_jquery_plugins.js', array('minimize' => true), 'smf_jquery_plugins');

		if (!User::$me->is_guest)
		{
			self::loadJavaScriptFile('jquery.custom-scrollbar.js', array('minimize' => true), 'smf_jquery_scrollbar');
			self::loadCSSFile('jquery.custom-scrollbar.css', array('force_current' => false, 'validate' => true), 'smf_scrollbar');
		}

		// script.js and theme.js, always required, so always add them! Makes index.template.php cleaner and all.
		self::loadJavaScriptFile('script.js', array('defer' => false, 'minimize' => true), 'smf_script');
		self::loadJavaScriptFile('theme.js', array('minimize' => true), 'smf_theme');
	...
	protected static function templateInclude($filename, $once = false)
	{
		...
		if ($file_found !== true)
		{
			...
			else
			{
				...
				echo sprintf(Lang::$txt['template_parse_error_details'], strtr($filename, array(Config::$boarddir => '', strtr(Config::$boarddir, '\\', '/') => '')), Config::$boardurl, Config::$scripturl);
