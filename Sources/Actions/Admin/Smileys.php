 ...
 */
class Smileys extends AbstractAction
{
			...
			if (self::$smileys_dir_found)
			{
				foreach (self::$smiley_sets as $smiley_set)
				{
					...
					if (!isset($filenames[$smiley_set['raw_path']]))
					{
						...
						Utils::$context['filenames'][$smiley_set['path']][''] = array(
							'id' => '',
							'selected' => true,
							'disabled' => true,
						);
	...
	public static function getConfigVars(): array
	{
		...
		$config_vars = array(
			...
			array('select', 'smiley_sets_default', array_map(fn($set) => $set['raw_name'], self::$smiley_sets)),
			array('check', 'smiley_sets_enable'),
			array('check', 'smiley_enable', 'subtext' => Lang::$txt['smileys_enable_note']),
			array('text', 'smileys_url', 40),
			array('warning', !is_dir(self::$smileys_dir) ? 'setting_smileys_dir_wrong' : ''),
			array('text', 'smileys_dir', 'invalid' => !self::$smileys_dir_found, 40),
			'',
	...
	protected function __construct()
	{
		...
		{
			unset(self::$subactions['addsmiley']);
			unset(self::$subactions['editsmileys']);
			unset(self::$subactions['setorder']);
			unset(self::$subactions['modifysmiley']);
		}

		if (empty(Config::$modSettings['messageIcons_enable']))
		{
			unset(self::$subactions['editicon']);
			unset(self::$subactions['editicons']);
		}
	...
	protected static function sanitizeString(string $string): string
	{
		...
		$string = Utils::htmlTrim(Utils::normalizeSpaces($string, true, true, array('no_breaks' => true, 'collapse_hspace' => true, 'replace_tabs' => true)));
