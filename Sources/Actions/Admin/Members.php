...
class Members implements ActionInterface
{
	 ...
	 *
	 */
	public array $activation_numbers = array();
	 ...
	 *
	 */
	public array $membergroups = array();
	...
	public static array $subactions = array(
		'all' => array('view', 'moderate_forum'),
		'approve' => array('approve', 'moderate_forum'),
		'browse' => array('browse', 'moderate_forum'),
	...
	public function browse()
	{
		...
		$browse_type = isset($_REQUEST['type']) ? $_REQUEST['type'] : (!empty(Config::$modSettings['registration_method']) && Config::$modSettings['registration_method'] == 1 ? 'activate' : 'approve');

		if (isset(Utils::$context['tabs'][$browse_type]))
			Utils::$context['tabs'][$browse_type]['is_selected'] = true;

		// Allowed filters are those we can have, in theory.
		$allowed_filters = $browse_type == 'approve' ? array(3, 4, 5) : array(0, 2);
		...
		$listOptions = array(
			...
			'get_items' => array(
				...
				'params' => array(
					...
					array('activated_status' => $this->current_filter),
					$show_duplicates,
				),
			),
			'get_count' => array(
		...
		if (!empty($show_filter) && !empty($available_filters))
		{
			$listOptions['additional_rows'][] = array(
				...
				'value' => '<strong>' . Lang::$txt['admin_browse_filter_show'] . ':</strong> ' . ((isset($this->current_filter) && isset(Lang::$txt['admin_browse_filter_type_' . $this->current_filter])) ? Lang::$txt['admin_browse_filter_type_' . $this->current_filter] : $available_filters[0]['desc']),
	 ...
	 */
	public static function list_getMembers($start, $items_per_page, $sort, $where, $where_params = array(), $get_duplicates = false)
	{
		$members = array();
	 ...
	 */
	public static function list_getNumMembers($where, $where_params = array())
	{
